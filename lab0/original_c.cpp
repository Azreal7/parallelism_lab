...
double* multiplication(double *mat1, double *mat2, double *mat, int m, int n, int k) {
    for(int i = 0; i < m; i++) {
        for(int j = 0; j < k; j++) {
            for(int l = 0; l < n; l++) {
...
double* multiplication_loopUnwinding(double *mat1, double *mat2, double *mat, int m, int n, int k) {
    for(int i = 0; i < m; i++) {
        for(int j = 0; j < k; j+=8) {
            for(int l = 0; l < n; l++) {
                mat[i * k + j] += mat1[i * n + l] * mat2[l * k + j];
...
// 修改循环顺序
double* new_multiplication(double *mat1, double *mat2, double *mat, int m, int n, int k) {
    for(int i = 0; i < m; i++) {
        for(int l = 0; l < n; l++) {
            for(int j = 0; j < k; j++) {
                mat[i * k + j] += mat1[i * n + l] * mat2[l * k + j];
...
int main() {
    ...
    double *res = multiplication(mat1, mat2, mat, m, n, k);
    auto end = chrono::high_resolution_clock::now();
    auto duration = chrono::duration_cast<chrono::microseconds>(end - start);
    cout << "Original Time: " << duration.count() << " microseconds\n" << endl;

    auto start2 = chrono::high_resolution_clock::now();
    double *res2 = new_multiplication(mat1, mat2, mat, m, n, k);
    auto end2 = chrono::high_resolution_clock::now();
    auto duration2 = chrono::duration_cast<chrono::microseconds>(end2 - start2);
    cout << "New Time: " << duration2.count() << " microseconds\n" << endl;

    auto start1 = chrono::high_resolution_clock::now();
    double *res1 = multiplication_loopUnwinding(mat1, mat2, mat, m, n, k);
