...
int main() {
    ...
    avg_rows = k / comm_sz;
    double *mat1, *mat2, *mat, *local_mat2, *local_mat3;
    double start, end;
    ...
    if (comm_sz == 1) {
        ...
        local_mat2 = (double *)malloc(avg_rows * n * 8);
        local_mat3 = (double *)malloc(avg_rows * k * 8);

        printf("#%d scattering.\n", my_rank);
        MPI_Scatter(mat2, avg_rows * k, MPI_DOUBLE, local_mat2, avg_rows * k, MPI_DOUBLE, 0, MPI_COMM_WORLD);

        printf("#%d Bcasting.\n", my_rank);
        MPI_Bcast(mat1, m * n, MPI_DOUBLE, 0, MPI_COMM_WORLD);

        matrix_multiply(avg_rows, n, k, local_mat2, mat2, local_mat3);
